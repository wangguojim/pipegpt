...
def top_k_logits(logits, top_k=0, top_p=0.0, filter_value=-float('Inf')):
    ...
    if top_k > 0:
        ...
        top_k, indices = torch.topk(logits, top_k)
        # tmp = top_k[..., -1, None]
        indices_to_remove = logits < top_k[..., -1, None]
...
def generate_samples_interactive(model, print_frequency=24):
    ...
    with torch.no_grad():
        while True:
            ...
            if mpu.is_pipeline_first_stage() \
               and mpu.get_tensor_model_parallel_rank() == 0:
                ...
                if "stop" in raw_text:
                    ...
                    context_tokens = tokenizer.encode(raw_text)[1:-1]
                    context_length = len(context_tokens)
            ...
            input_info = [terminate_runs, raw_text_len, context_length]  # 中断运行标志， 原始文本长度，token化之后长度
            input_info_tensor = torch.cuda.LongTensor(input_info)
            torch.distributed.all_reduce(input_info_tensor,  # 同步到其他GPU
                                         group=mpu.get_model_parallel_group())
            terminate_runs = input_info_tensor[0].item()  # 中断运行标志
            raw_text_len = input_info_tensor[1].item()  # 原始文本长度
            context_length = input_info_tensor[2].item()  # token编码后长度

            if terminate_runs == 1:  # 如果中断运行
                return  # 返回None

            # For pipeline parallel we send context tokens to other stages
            # so they get the lengths correct
            if mpu.get_tensor_model_parallel_rank() == 0 \
               and args.pipeline_model_parallel_size > 1:  # tensor rank==0，pp>1
                if mpu.is_pipeline_first_stage():  # first stage
                    src = mpu.get_pipeline_model_parallel_first_rank()
                    group = mpu.get_pipeline_model_parallel_group()
                    context_tokens_tensor = torch.cuda.LongTensor(context_tokens)
                    torch.distributed.broadcast(context_tokens_tensor, src, group)
            ...
            for counter, decode_tokens in enumerate(token_stream):
                ...
                # trim_decode_tokens = tokenizer.detokenize(
                #     decode_tokens)[raw_text_len:]
                trim_decode_tokens = tokenizer.convert_ids_to_tokens(
                    decode_tokens)[raw_text_len:]
                print("\nMegatron-LM:", trim_decode_tokens, flush=True)
...
def generate_samples_unconditional(model, latencies=[], model_latencies=[], single_token_latency=[]):
    ...
    context_tokens = [[tokenizer.sep_token_id]
...
def switch(val1, val2, boolean):
    ...
    tmp = (1 - boolean) * val1 + boolean * val2
    ...
    with torch.no_grad():
        ...
        while context_length <= (maxlen):
            ...
            if mpu.is_pipeline_last_stage():
                ...
                tmp = tokens[:, context_length].view(-1)
                new_tokens = switch(
