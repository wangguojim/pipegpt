...
def pretrain(train_valid_test_dataset_provider,
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        ...
        train_data_iterator, valid_data_iterator, test_data_iterator \
            = build_train_valid_test_data_iterators(  # 本文件1074行
                train_valid_test_dataset_provider)  # 调用pretrain_gpt.py第227行，调用gpt_dataset.py第31行
...
def get_model(model_provider_func):
    ...
    if mpu.get_pipeline_model_parallel_world_size() > 1 and \
       args.virtual_pipeline_model_parallel_size is not None:
        ...
        pre_process = mpu.is_pipeline_first_stage()  # first stage flag
        post_process = mpu.is_pipeline_last_stage()  # last stage flag
        model = model_provider_func(  # 某一stage
            pre_process=pre_process,
            post_process=post_process
        )
...
def build_train_valid_test_data_iterators(
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if args.train_samples:
            ...
            train_samples = args.train_iters * args.global_batch_size  # 计算出一共需要的train_samples
        eval_iters = (args.train_iters // args.eval_interval + 1) * \
                     args.eval_iters
        test_iters = args.eval_iters
        train_val_test_num_samples = [train_samples,  # 训练需要的sample个数
                                      eval_iters * args.global_batch_size,  # 验证需要的sample个数
                                      test_iters * args.global_batch_size]  # 测试需要的sample个数
        ...
        # Build the datasets.
        train_ds, valid_ds, test_ds = build_train_valid_test_datasets_provider( #调用pretrain_gpt.py第227行，调用gpt_dataset.py第31行
