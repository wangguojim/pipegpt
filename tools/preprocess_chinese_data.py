...
class Encoder:
    def __init__(self, args):
        self.args = args
        #             print(args.special_token_file)

        special_tokens = yaml.load(open(args.special_token_file), Loader=yaml.FullLoader)['special_tokens']
        self.tokenizer = BertTokenizer.from_pretrained(self.args.vocab_file,
                                                       additional_special_tokens=special_tokens)  # 用中文词汇表以及特殊词表初始化tokenlizer
    ...
    def encode(self, json_line):
        # try:
        data = json.loads(json_line, strict=False)  # 读入字符串格式的json内容，使用json.loads将其转化为json格式
        # except Exception as e:
        #     return '1', '1'
        ids = {}
        for key in self.args.json_keys:  # 在转化好的json内容中根据key找到需要处理的部分
            text = data[key]  # 将每个需要处理的部分储存在text变量中
            doc_ids = []
            texts = [i.strip() for i in text.split('#&#') if i.strip()]
            for text in texts:
                sentence_ids = self.tokenizer.encode(text)  # 使用刚刚初始化的tokenlizer将文本内容编码
                if len(sentence_ids) > 0:
                    doc_ids.append(sentence_ids[1:-1])  # 编码后的句子存入doc_ids
            if len(doc_ids) > 0 and self.args.append_eod:
                doc_ids[-1].append(self.tokenizer.sep_token_id)
...
def get_args():
    ...
    group.add_argument('--special-token-file', type=str, default=None,
                       help='Path to the vocab file')
    ...
    group.add_argument('--workers', type=int, default=1,  # 并行数
                       help='Number of worker processes to launch')
    group.add_argument('--log-interval', type=int, default=100,  # 每隔多少行输出一次
...
def main():
    ...
    #pool = multiprocessing.Pool(initializer=encoder.initializer)
    #     encoded_docs = pool.imap(encode, fin, 25)  # 并行处理,结果存到内存
    encoded_docs = pool.imap(encoder.encode, fin)  # 对文件内容调用encoder中的encode方法
    # 返回结果的可迭代对象，并行处理,结果存到内存
