...
DIR=`pwd`
OUTPUT_BASEPATH='/raid/nlp/ckpts/ckpt-gpt-1p3-MoE-24'
###############################################################################
### Main configs
...
TRAIN_TOKENS=196608000
#TRAIN_TOKENS=491520000
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    DATA_BLEND="/raid/nlp/data/aggregated_meta_data/dataset_10b_22_text_document"
fi
###############################################################################
data_options=" \
...
##template_json="./examples/MoE/ds_config_gpt_Zero2_TEMPLATE.json"
...
cat <<EOT > $config_json
{
  ...
  "curriculum_learning": {
    ...
    "min_difficulty": $CL_START_SEQLEN,
    "max_difficulty": $SEQ_LEN,
    "schedule_type": "fixed_linear",
    "schedule_config": {
      "total_curriculum_step": $CL_STEP,
      "difficulty_step": 8
    }
  },
  "comms_logger": {
...
#run_cmd="deepspeed --hostfile hostfile ${DIR}/pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} &> ${OUTPUT_BASEPATH}/log/${NAME}_${host}_${current_time}.log"
run_cmd="deepspeed --hostfile hostfile ${DIR}/pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} &> kaiwu_${NAME}_${host}_${current_time}.log"
#run_cmd="deepspeed --hostfile hostfile ${DIR}/pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}"
